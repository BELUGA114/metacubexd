...
> published as a `metacubexd-gh-pages.zip` asset on this repository's releases.
>
...
Download `metacubexd-gh-pages.zip` from the
...
unzip -q metacubexd-gh-pages.zip -d /etc/mihomo/ui
```
...
from any path, then attaches `metacubexd-gh-pages.zip` to a GitHub release. To
...
output, or as the `metacubexd-gh-pages.zip` release asset — **must retain the
